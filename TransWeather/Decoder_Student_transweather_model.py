...
class EncoderTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, 
                 ...
                 in_chans=3, 
                 num_classes=1000, 
                 embed_dims=[32, 64, 128, 256], #Modificado de [64, 128, 256, 512] a [32, 64, 128, 256]
                 num_heads=[1, 2, 4, 8], #Posible modificación
                 mlp_ratios=[4, 4, 4, 4], 
                 qkv_bias=False, 
                 qk_scale=None, 
                 drop_rate=0.,
                 ...
                 depths=[3, 2, 3, 3], #Modificado de [3, 4, 6, 3] a [3, 2, 3, 3]
                   sr_ratios=[8, 4, 2, 1]):
        super().__init__()
...
class Attention(nn.Module):
    def __init__(self, 
                dim, 
                num_heads=4, #Modificado de 8 a 4
                qkv_bias=False, 
                qk_scale=None, 
                attn_drop=0., 
                proj_drop=0., 
...
class Tenc(EncoderTransformer):
    def __init__(self, **kwargs):
        super(Tenc, self).__init__(
            patch_size=2, #modificado de 4 a 2 
            embed_dims=[64, 128, 320, 512], #Error al modificar de [64, 128, 320, 512] a [32, 64, 128, 256]
            num_heads=[1, 2, 2, 4], #Modificado de [1, 2, 4, 4] a [1, 2, 2, 4]
            mlp_ratios=[2, 2, 2, 2], #Posible modificación
            qkv_bias=True, 
...
class Tdec(DecoderTransformer):
    def __init__(self, **kwargs):
        super(Tdec, self).__init__(
            patch_size=2, #modificado de 4 a 2
            embed_dims=[64, 128, 320, 512], #Error al modificar de [64, 128, 320, 512] a [32, 64, 128, 256]
            num_heads=[1, 2, 2, 4], #Modificado de num_heads=[1, 2, 5, 8] a [1, 2, 2, 4]
            mlp_ratios=[2, 2, 2, 2], #Modificado de [4, 4, 4, 4] a  [2,2,2,2]
            qkv_bias=True, 
            norm_layer=partial(nn.LayerNorm, eps=1e-6), 
            depths=[2, 1, 3, 2], #Modificado de [3,4,6,3] a [2,1,3,2]
            sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, 
            drop_path_rate=0.1)
...
class Student_Transweather(nn.Module):

    def __init__(self, path=None, **kwargs):
        super(Student_Transweather, self).__init__()
        self.Tenc = Tenc()
        self.Tdec = Tdec()
        self.convtail = convprojection()
        self.clean = ConvLayer(8, 3, kernel_size=3, stride=1, padding=1)#error al modificar de 8, 3 a 4,3 #Posible modificación (in_channels, out_channels, kernel_size, stride, padding)
        self.active = nn.Tanh()
        if path is not None:
            self.load(path)
