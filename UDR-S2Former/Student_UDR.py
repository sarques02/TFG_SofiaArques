...
import math 
import time
from base_net_snow_student import *
...
class SparseSamplingAttention(nn.Module):
    ...
    def get_constraint_matrix(self, x, var_3d):
        b, c, h, w = x.shape
        mask = torch.zeros(b, c, h * w, dtype=torch.bool, device=x.device)
        # Verifica si el tamaño de la dimensión es menor que k
        if var_3d.size(-1) < self.topk:
            # Si es menor, ajusta k para que no sea mayor que el tamaño de la dimensión
            k = var_3d.size(-1)
        else:
            k = self.topk
        index = torch.topk(var_3d, k=k, dim=-1, largest=True)[1]
        # Actualiza la máscara
        mask.scatter_(-1, index, 1)
        constraint_matrix = torch.where(mask, torch.full_like(var_3d, 1), torch.full_like(var_3d, 0.6))
...
class Transformer_Student(nn.Module):
    ...
    in_channels=3,
    out_cahnnels=3,
    transformer_blocks = 4, #Modificado de 8 a 4
    ...
    drop_path_=0.,
    sr=1,
    conv_num = [1,2,4,6], #Modificado de [4,6,7,8] a [1,2,4,6]
    expand_ratio = [2,2,2,2],
    VAN = False,
    dynamic_e = False,
    global_atten = True,
    ):
        super(Transformer_Student,self).__init__()
        self.patch_size = patch_size
